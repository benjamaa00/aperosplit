...
import {
  ...
  Pencil, Check, X, Tag, Search, Image as ImageIcon,
} from "lucide-react";
...
const PRESET_COLORS = [
  ...
  "#a855f7","#ef4444","#84cc16","#06b6d4","#d946ef",
  "#f59e0b","#10b981","#6366f1","#e11d48","#0891b2",
];
...
const EmojiPicker = memo(function EmojiPicker({ onSelect, onClose }: EmojiPickerProps) {
  ...
  const [activeCat, setActiveCat] = useState("Populaire");
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="bg-background rounded-t-3xl sm:rounded-2xl w-full max-w-sm shadow-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="px-4 pb-2">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Rechercher ou coller un emoji..."
              autoFocus
              className="w-full bg-card/50 border border-border rounded-xl pl-8 pr-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary/30"
            />
          </div>
        </div>
        {!search.trim() && (
            ...
            {Object.keys(EMOJI_CATEGORIES).map(cat => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold whitespace-nowrap transition-colors ${
                  activeCat === cat ? "bg-primary/15 text-primary" : "bg-card/50 text-muted-foreground"
                }`}
              >
                {cat}
              </button>
        ...
        <div className="grid grid-cols-8 gap-1 p-4 pt-0 overflow-y-auto flex-1 min-h-0">
          ...
          {filtered.length === 0 && (
            <p className="col-span-8 text-xs text-muted-foreground text-center py-6">Aucun résultat</p>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          className="w-8 h-8 rounded-full border-2 border-dashed border-border flex items-center justify-center hover:scale-105 transition-all"
      ...
      {value && (
        <button onClick={() => onSelect(value)} className="text-[10px] text-muted-foreground hover:text-foreground">
          Retirer la couleur
        </button>
      )}
    </div>
    ...
    >
      <div className="p-4">
        <div className="flex items-center gap-3">
          ...
          >
            {catImage ? (
              <img src={catImage} alt="" className="w-8 h-8 rounded-lg object-cover shrink-0" />
            ) : (
...
export const CategoryManagementScreen = memo(function CategoryManagementScreen({
  ...
  const resetForm = () => { setNewName(""); setNewEmoji("📦"); setNewColor(undefined); setNewIcon(null); };
          ...
          <div className="flex items-center gap-3">
            <div className="flex flex-col items-center gap-1 shrink-0">
              <button
                ...
                className="w-14 h-14 rounded-2xl bg-card border border-border flex items-center justify-center text-3xl shrink-0 overflow-hidden"
              >
                {newIcon ? (
                  <img src={newIcon} alt="" className="w-full h-full object-cover" />
                ) : (
                  newEmoji
                )}
              </button>
