...
function SettingsTab({ groupName, requireApproval, onUpdateSettings, onResetAllData }: {
 ...
 <button onClick={() => onUpdateSettings?.({ name })}
 ...
 <button onClick={() => { setShowResetConfirm(false); onResetAllData?.(); }}
...
function MemberDetail({ selectedMember, currentMemberId, memberStats, memberRecentExpenses, isAdmin, onChangeRole, setShowConfirmExpel, setTab }: MemberDetailProps) {
 ...
 return (
 <div {...fadeSlide} className="space-y-5">
 ...
 <div className="w-20 h-20 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center border-2 border-primary/30 mx-auto shadow-xl shadow-primary/10"
 >
 ...
 <div className="space-y-2">
 <button onClick={() => onChangeRole?.(selectedMember.id, isAdmin(selectedMember.id) ? "member" : "admin")}
 ...
 <button onClick={() => setShowConfirmExpel(selectedMember.id)}
...
export function MemberManagement({
 ...
 <button onClick={() => { setShowAddMember(true); setAddMemberName(""); setAddMemberResult(null); setShowPin(false); }}
 ...
 <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary/15 to-primary/5 flex items-center justify-center border border-primary/20 shadow-md shadow-primary/5">
 ...
 <>
 <button onClick={() => setShowRoleMenu(showRoleMenu === member.id ? null : member.id)}
 className="w-8 h-8 rounded-xl bg-muted/30 flex items-center justify-center">
 <Shield size={14} className="text-muted-foreground" />
 </button>
 <button onClick={() => setShowConfirmExpel(member.id)}
 ...
 <div className="flex gap-2 mt-6">
 <button onClick={() => setShowConfirmExpel(null)}
 className="flex-1 py-3 rounded-2xl bg-card/30 border border-border text-sm font-semibold">
 Annuler
 </button>
 <button onClick={() => { onRemoveMember(showConfirmExpel); setShowConfirmExpel(null); setTab("list"); }}
 className="flex-1 py-3 rounded-2xl bg-red-500 text-white text-sm font-semibold">
 Expulser
 </button>
 </div>
 ...
 <button onClick={() => { setShowAddMember(false); setAddMemberResult(null); }}
 ...
 <div className="flex gap-2 mt-4">
 <button onClick={() => handleSharePin(addMemberResult.name, addMemberResult.accessPin)}
 ...
 <button onClick={() => { handleCopyPin(addMemberResult.accessPin); }}
 ...
 <button onClick={() => { setShowAddMember(false); setAddMemberResult(null); setShowPin(false); }}
