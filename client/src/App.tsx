...
export default function App() {
 ...
 if (saved === "home" || saved === "balances" || saved === "history" || saved === "profile" || saved === "messages") return saved;
 return "home";
 });
 ...
 return b;
  ...
  const suggestedTransactions = useMemo(() => simplifyDebts(balances, pendingPayments), [balances, pendingPayments]);
 ...
 content = (
  ...
  <div key={`tab-anim-${activeTab}`} className={`tab-content-animate ${activeTab === "messages" ? "h-full" : ""}`}>
  ...
  {activeTab === "balances" && <BalancesTab key="balances" members={members} balances={balances} suggestedTransactions={suggestedTransactions} currentMemberId={currentMemberId} onRequestPayment={(toId: string, amount: number, note?: string) => requestPayment(toId, amount, undefined, note)} expenses={expenses} currency={currency} completedPayments={completedPayments} />}
  ...
  {activeTab === "stats" && <ErrorBoundary><StatsTab key="stats" expenses={expenses} members={members} currentMemberId={currentMemberId} pendingPayments={pendingPayments} completedPayments={completedPayments} monthlyBudget={monthlyBudget} currency={currency} categories={getCategoriesQuery.data?.categories || []} /></ErrorBoundary>}
   ...
   {activeTab === "messages" && <MessagesTab key="messages" currentMemberId={currentMemberId} members={members} currency={currency} isAdmin={currentMember?.role === "admin"} />}
  </div>
